...
rownames(samp_df) <- samp_df[[1]]   # First column contains sample IDs
samp_df <- samp_df[,-1]             # Remove the sample ID column
...
tax_mat <- tax_mat[,-1]              # Remove feature ID column from matrix
...
C_M_inflamed       <- subset_samples(inflamed, Medications %in% c("Mesalamine+corticosteroids", 
                                                                  "corticosteroids+Mesalamine"))
C_M_noninflamed    <- subset_samples(noninflamed, Medications %in% c("Mesalamine+corticosteroids", 
                                                                     "corticosteroids+Mesalamine"))
...
# C + Combine vs No (multi-drug combos including C or M)
C_Comb_inflamed <- subset_samples(inflamed, Medications %in% c(
  "Mesalamine+corticosteroids+mercaptopurine",
  "Mercaptopurine+corticosteroids"
))
C_Comb_noninflamed <- subset_samples(noninflamed, Medications %in% c(
  "Mesalamine+corticosteroids+mercaptopurine",
  "Mercaptopurine+corticosteroids"
))
#### Merge phyloseq objects for comparisons ####
# C vs M
...
nsamples(C_inflamed); nsamples(M_inflamed); nsamples(C_M_inflamed); nsamples(No_inflamed)

#### Helper function: safe plot_bar with Family fallback for NA Genus ####
plot_core <- function(physeq, group_name) {
  # Replace NA Genus with "Unclassified_Family"
...
#### G1: Corticosteroids only vs All Inflamed/Noninflamed ####
